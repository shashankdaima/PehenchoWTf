const Footer=()=>{
    return (
        <div className="mx-auto bg-black !text-white mt-12 py-8 flex flex-col space-y-2 justify-center items-center border-t-4  border-black">
          <h4 className="bangers-regular text-3xl">
            Thanks for visiting this page!
          </h4>
          <p className="zilla-slab text-center max-w-lg px-8 text-md md:text-lg">
            <strong>(##)</strong> It was a fun little project, not intended to
            harm anyone. If I find anyone engaging in any sort of
            discrimination/harassment, those posts will be deleted.
          </p>
        </div>
    );
}
export default Footer;